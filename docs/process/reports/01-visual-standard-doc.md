...
- **后续演进（01:14 取证）**：票 03 已**将其分支显式叠于票 01 之上**——`STACK k0`：`ui-craft/03-button-system-cleanup [qws, vns, yoo]` ← `ui-craft/01-visual-standard-doc [sln, xov]`；票 03 提交信息自述「依赖票 01 分支（ui-visual-standard.md v1.0）故本分支已叠于其上」。`but branch list` 现显示两票同为 `✗`（`mergesCleanly: false`），票 04 仍为 `✓`。
- **影响面上调**：§4.5-D-1 的 `mergesCleanly: false` 已**不再是票 01 自身的单点问题**，它同时阻塞票 03 的合入。建议大脑把该遗留项的优先级由「票 01 收尾」上调为「阻塞两票的合流前置」。

### 4.7 呈报：一条想当然路径已证伪 —— mergesCleanly 无法靠改写分支内容翻转

- **假设**：预先把上游 CONTEXT.md 的改动折进本票分支，即可让 mergesCleanly 由 false 转 true。
- **执行**：据此手工完成三方合并——上游区块（Composition Root / WindowPollingHostedService）原位保留 + 本票区块（UI Visual Standard / Visual Baseline）接后 + Sidebar Nav Item 44px→40px。工作区得 **42,862 字节**，与 pull 期间手工解析结果**逐字节吻合**（两次独立操作互证，解析内容无误）。
- **结果**：提交后 mergesCleanly **仍为 false**（票 01 与票 03 皆然；票 04 仍 true）。假设证伪。
- **原因（已取证）**：冲突点是 ServiceModeController 词条之后的**同点插入**。git 的 merge-file **不做超集识别**——即便我方区块已完整包含对方区块，只要双方在同一位置插入长度不同的内容，仍判 1 处冲突；该冲突只有在本票被 rebase 到上游之上后才会消失。故该标志位反映的是**结构事实**（本分支需 rebase 到 6 个上游提交之上且双方触及同一区段），**不是内容缺陷**，改写分支内容无法翻转。
- **附带取证（易被误判为数据丢失，实为正常）**：本票分支提交 mns 的 CONTEXT.md 为 **42,660 字节**，工作区为 **42,862 字节**，差 202 字节。逐行 diff 确认差异**仅限第 53 / 61 / 62 行**三条发布相关条目——属**上游改动**，本票分支只需记录本票自身改动，rebase 后由上游提供；工作区版本含它们是因为工作区 WIP 已先行同步。两者在各自语境下都正确，**非丢失**。
- **结论（修正 §4.5-D-1）**：选项 **(b)「改写本票内容以消除冲突」已证伪，不再推荐**。可行路径收缩为 (a) 由大脑在 GitButler 之外完成 rebase（原生 git rebase 或 GUI）后本票重放；(c) 维持现状、待 land 时一次性解析。
- **方法教训**：本轮一度因 git merge-file 配合进程替换（`<(git show ...)`）读取失败、抛错却 stdout 为空，被误读为「0 冲突」。已改用真实临时文件复算，结论以复算为准。**凡「干净」结论必须有退出码佐证，不能只看冲突标记计数。**
