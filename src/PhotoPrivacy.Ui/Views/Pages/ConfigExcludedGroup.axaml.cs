...
// 票 04（ui-craft2）返工轮：ConfigPage 排除目录组子件。访问器签名与票 04 原 ConfigPage 同名，
// 由 ConfigPage.axaml.cs 转发给 MainWindow.axaml.cs（接线零改动）；无事件订阅（Pages 目录规约）。
public partial class ConfigExcludedGroup : UserControl
{
    public ConfigExcludedGroup() => InitializeComponent();
